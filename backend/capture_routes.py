...
数据与流程（与留言模块完全隔离）：
  ...
  - 手动抓拍：App POST /api/ezviz/capture → 后端调萤石 /api/lapp/device/capture
    ...
    或 App EZOpenSDK 本地抓帧后 POST /api/ezviz/capture/upload（base64 JSON，
    python-multipart 未安装故不走 multipart）直接上传落盘（共享同一 4s 限流器）。
...
class CaptureUploadRequest(BaseModel):
    """SDK 本地抓帧上传（base64 JSON；eventTime 可空，后端兜底当前时间）"""
    device_serial: str = Field("", alias="deviceSerial")
    channel_no: int = Field(1, alias="channelNo")
    event_time: int = Field(0, alias="eventTime")
...
@router.post("/api/ezviz/capture/upload")
async def upload_capture(req: CaptureUploadRequest):
    """手动抓拍图片上传：App EZOpenSDK 本地抓帧后上传，落盘 alarm_events(manual)。

    与 /api/ezviz/capture 共享同一 4s 限流器（后端为唯一事实源，App 侧仅防连点）。
    不推 WS（App 是发起方，自己拿到结果）。
    错误码：-1 参数/数据错误 / -2 4s 限流 / -5 图片无效或过大或保存失败。
    """
    device_serial = (req.device_serial or "").strip().upper()
    if not device_serial:
        return _err(-1, "deviceSerial 不能为空")
    if not req.image_base64:
        return _err(-1, "imageBase64 不能为空")
    ...
    # 1) base64 解码 + 体积上限校验
    try:
        file_bytes = base64.b64decode(req.image_base64, validate=True)
    except Exception as e:
        logger.warning("[抓拍上传] base64 解码失败: %s", e)
        return _err(-1, "图片数据无效")
    if not file_bytes or len(file_bytes) > MAX_PIC_BYTES:
        return _err(-5, "图片数据无效或过大")
    ...
    if not rel_path:
        return _err(-5, "图片保存失败")
    event_time = req.event_time or int(time.time() * 1000)
    await asyncio.to_thread(
        db.save_capture,
        record_id, "manual", device_serial, event_time,
        "", "手动抓拍", "", rel_path, "{}",
    )
    logger.info("[抓拍上传] 手动抓帧上传成功 recordId=%s 设备=%s 图片=%s", record_id, device_serial, rel_path)
