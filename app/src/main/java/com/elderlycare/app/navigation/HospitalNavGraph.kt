...
fun HospitalMainScreen(navController: NavHostController, onLogout: () -> Unit) {
    ...
    Scaffold(
                            ...
                            onClick = {
                                if (currentRoute != item.route) {
                                    innerNavController.navigate(item.route) {
                                        // 欢迎页成为 startDestination 后，startDestinationId 会指回欢迎页，
                                        // 故硬编码 popUpTo("hospital_emergency") 防止 Tab 切换救回欢迎页
        ...
        NavHost(
            ...
            // 医院端欢迎页（点击进入急救大屏，清空欢迎页栈，返回键不回欢迎页）
                ...
                QualificationGate {
                    HospitalEmergencyPanelScreen(
                        ...
                        onUserClick = { elderlyId ->
                            // 空 elderlyId 不导航（路由不匹配会崩溃）；详情走根图 UserDetail 路由
                ...
                QualificationGate {
                    HospitalHealthRecordsScreen(
                        onNavigateToDetail = { elderlyId ->
                            // 空 elderlyId 不导航（路由不匹配会崩溃）
                            if (elderlyId.isNotBlank()) {
                                navController.navigate(Screen.UserDetail.createRoute(elderlyId))
                            }
                        },
                        // 档案页随访保持医院版 MedicalFollowUpScreen（按老人录入，BASE 已交付业务）；
                        // 社区版随访计划由「我的」页入口进入（hospital_followup_plan），两套并存
