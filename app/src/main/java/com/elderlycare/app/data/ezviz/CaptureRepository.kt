 ...
 *
 * 链路（手动抓拍）：预览页截图按钮 → CaptureFrameSession 本地抓帧 →
 * App Base64 上传后端 /api/ezviz/capture/upload → 落盘 alarm_events(manual)
 * （4s 限流由后端保证）→ 全部抓拍页展示。
...
class CaptureRepository(
    ...
    /**
     * 手动抓拍图片上传：SDK 本地抓帧文件 → Base64 → 后端 /api/ezviz/capture/upload。
     * 成功返回「截图已保存」文案；失败 message 为 toast 文案。
     */
    suspend fun uploadFrame(deviceSerial: String, file: File): Result<String> = runCatching {
        val bytes = file.readBytes()
        if (bytes.isEmpty()) throw CaptureException("图片数据无效")
        val resp = api.uploadCaptureFrame(
        ...
        Log.d(TAG, "抓帧上传成功 device=$deviceSerial")
        "截图已保存"
    }

    /**
     * 手动云端抓拍（旧链路：后端→萤石 device/capture→下载落盘 alarm_events(manual)→
     * 回传 localPicUrl→App 下载存相册）。已由 [uploadFrame]（SDK 本地抓帧）取代，
     * 保留作回退。成功返回「截图已保存」文案；失败 message 为 toast 文案。
     */
    @Deprecated("由 uploadFrame（SDK 本地抓帧）取代，保留作回退")
    suspend fun capture(deviceSerial: String): Result<String> = runCatching {
        ...
        Log.d(TAG, "手动云端抓拍成功 device=$deviceSerial url=$localPicUrl")
        "截图已保存"
    }
