...
object RtcSignalingManager {
    ...
    private fun handleAlarm(data: JSONObject) {
        ...
        runCatching {
            runBlocking {
                ServiceLocator.messageRepository.saveAlertMessages(listOf(alarm))
                // 三端一 App 共享异常状态：按 deviceSerial 反查档案 →
                // 本地告警表（LocalAlertEntity，社区花名册「异常」角标数据源）+ 档案 alertActive
                val profile = ServiceLocator.profileStore.getAllProfiles()
                    .firstOrNull { it.deviceSn == deviceSerial }
                if (profile != null) {
                    ServiceLocator.bindingDao.insertAlert(
                        LocalAlertEntity(
                            id = alarmId,
                            deviceId = deviceSerial,
                            elderlyId = profile.userId,
                            type = alarm.alarmName,
                            level = AlertLevel.ATTENTION.name,
                            content = alarm.alarmName,
                            timestamp = System.currentTimeMillis(),
                            status = AlertStatus.UNREAD.name
                        )
                    )
                    ServiceLocator.profileStore.saveProfile(profile.copy(alertActive = true))
                }
            }
        }.onFailure { e ->
