...
interface MessageDao {
    ...
    /**
     * 绑定申请未读数（我的页授权管理红角标数据源）。
     * SQLite LIKE 无 ESCAPE 子句，字面下划线用 GLOB 'binding_*' 精确匹配前缀。
     * 不按 deviceSerial 过滤：绑定消息归属该家属账号（申请按 elderlyId 生成，
     * 设备 SN 可能为空/变更，按 SN 过滤会漏计）。
     */
    @Query("SELECT COUNT(*) FROM message WHERE msgType = 4 AND remoteId GLOB 'binding_*' AND isRead = 0")
    fun observeUnreadBindingCount(): kotlinx.coroutines.flow.Flow<Int>

    /** 进入授权管理页批量标记绑定申请消息已读（红角标随之消失） */
    @Query("UPDATE message SET isRead = 1 WHERE msgType = 4 AND remoteId GLOB 'binding_*' AND isRead = 0")
    suspend fun markBindingMessagesRead()
}
