 ...
 * - v6：复诊双重确认——remind_plan 表新增 confirmStatus 列
 *   （0=无确认流程/1=待家属确认/2=已同意/3=已拒绝），见 AppMigrations.MIGRATION_5_6
 * - v7：社区端业务——新增 community_follow_up（社区随访）、staff_schedule（排班）、
 *   service_record（服务记录）、todo_item（待办事项）四张表，见 AppMigrations.MIGRATION_6_7
        ...
        fun getInstance(context: Context): AppDatabase {
            return instance ?: synchronized(this) {
                instance ?: Room.databaseBuilder(
                    ...
                    // v5→v6 remind_plan.confirmStatus 复诊双重确认；
                    // v6→v7 社区端随访/排班/服务记录/待办四张表）
                    .addMigrations(
