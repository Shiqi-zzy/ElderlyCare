...
interface RtcBackendApi {
    ...
    /** 手动抓拍图片上传（SDK 本地抓帧 → 后端落盘 alarm_events(manual)；base64 JSON） */
    @POST("api/ezviz/capture/upload")
    suspend fun uploadCaptureFrame(@Body body: CaptureUploadRequest): BackendBaseResponse
    ...
    // ===== 健康档案云同步（后端 profile_routes：家属上传 / 机构端拉取）=====

    /** 健康档案上传（家属端保存档案后 fire-and-forget；失败静默降级） */
    @POST("api/health-profile/upload")
    suspend fun uploadHealthProfile(@Body body: HealthProfileUploadRequest): BackendBaseResponse

    /** 健康档案拉取（机构端授权后查看；失败静默回退本地） */
    @GET("api/health-profile/{userId}")
    suspend fun getHealthProfile(@Path("userId") userId: String): BackendBaseResponse
}
...
/** 手动抓拍图片上传请求体（后端 POST /api/ezviz/capture/upload；imageBase64 = 抓帧 jpg 字节 Base64） */
...
/** 健康档案上传请求体（后端 POST /api/health-profile/upload；profileJson = Gson 序列化档案原文） */
data class HealthProfileUploadRequest(
    val userId: String = "",
    val deviceSn: String = "",
