...
import com.elderlycare.app.data.profile.HealthProfileCloudRepository
...
import com.elderlycare.app.data.rk3.Rk3LanRepository
...
object ServiceLocator {
    ...
    // ===== 健康档案云同步（自家后端：家属上传/机构端拉取）=====
    lateinit var healthProfileCloudRepository: HealthProfileCloudRepository
        private set

    // ===== RK3 局域网报告/建议（动态 baseUrl + 12s 超时独立客户端，不复用 30s 共享 client）=====
    lateinit var rk3Repository: Rk3LanRepository
        private set
    ...
    fun init(context: Context) {
        ...
        // ===== 健康档案云同步（复用同一自建后端 Retrofit）=====
        healthProfileCloudRepository = HealthProfileCloudRepository(rtcBackendApi)

        // ===== RK3 局域网报告/建议（12s 超时独立客户端：局域网快速失败不拖慢降级反馈）=====
        val rk3HttpClient = OkHttpClient.Builder()
            .addInterceptor(logging)
            ...
            .build()
        rk3Repository = Rk3LanRepository(settingsStore, Rk3LanClient(rk3HttpClient))
        ...
        // bindingRepository 需注入 messageDao（绑定申请 → 家属 SYSTEM 消息），故置于 AppDatabase 构建之后
        bindingRepository = BindingRepository(staffUserStore, userStore, profileStore, bindingDao, appDatabase.messageDao())
