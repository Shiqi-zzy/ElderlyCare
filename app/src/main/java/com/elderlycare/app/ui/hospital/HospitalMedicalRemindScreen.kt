 ...
 * 医院端「复诊提醒」。
 *
 * 照护对象选择（FilterChip 行，含未授权照护对象）+ 医院端计划列表 + 浮动按钮打开录入表单。
 * 表单：提醒日期/时间 + ≤20 字文本 + 复选框【下发设备进行播报】——
 * 强制授权校验：无 ACTIVE 医院授权 → 复选框置灰 + toast「暂无家属授权，不能下发提醒到设备」；
 * 有授权但未绑定设备 → 置灰 + toast「该照护对象未绑定设备，不能下发播报」。
 *
...
/** 单条医院端复诊提醒计划：内容 + 时间/重复文案 + 来源 chip + 已播报 chip + 所属照护对象 */
@Composable
    ...
    Card(
        ...
        Column(modifier = Modifier.padding(14.dp)) {
            ...
            Row(verticalAlignment = Alignment.CenterVertically) {
                ...
                // 复诊双重确认状态（只读小徽标，家属端操作在提醒计划页）
                val confirmBadge = when (plan.confirmStatus) {
                    ...
                    RemindPlanEntity.CONFIRM_REJECTED ->
                        R.string.reminder_confirm_rejected to StatusRed
                    else -> null
                }
                if (confirmBadge != null) {
                    Surface(
                        shape = RoundedCornerShape(6.dp),
                        color = confirmBadge.second.copy(alpha = 0.12f)
                    ) {
                        Text(
                            stringResource(confirmBadge.first),
                            modifier = Modifier.padding(horizontal = 6.dp, vertical = 1.dp),
                            style = MaterialTheme.typography.labelSmall,
                            color = confirmBadge.second
                        )
                    }
                    Spacer(Modifier.width(8.dp))
                }
