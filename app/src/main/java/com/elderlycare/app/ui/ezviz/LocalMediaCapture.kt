 ...
 *
 * ⚠️ 边界区分：
 * - 这是手机本地录制/抓帧，不是设备 SD 卡回放下载（勿与回放搜索/下载接口混淆）；
 * - 手动抓拍 = EZOpenSDK 本地抓帧（CaptureFrameSession 隐藏会话 → capturePicture），
 *   抓帧文件上传自家后端落 alarm_events(manual)（CaptureRepository.uploadFrame）；
 *   旧「App → 后端 → 萤石 device/capture」云端链路接口保留作回退；
 ...
 * - 抓帧：capturePicture(path) 为同步 int 返回（0/-1），SDK 无 MSG_PICTURE_SAVED 消息码，
 *   返回 0 后仍需轮询文件落盘（部分设备异步写盘）。
 */
object LocalMediaCapture {
    ...
    /**
     * 隐藏 EZPlayer 抓帧会话（手动抓拍 = EZOpenSDK 本地抓帧）。
     *
     * 生命周期：
     * start（createPlayer + setPlayVerifyCode + setSurfaceHold + setHandler + startRealPlay）
     *  → 播放成功回调（MSG_REALPLAY_PLAY_SUCCESS=102）→ capturePicture(absolutePath) 抓帧
     *  → 轮询文件落盘 → stopRealPlay + releasePlayer → [Listener.onCaptured](file)。
     * 任何一步失败/超时都会回收会话并回调 [Listener.onFailed]。
     *
     * ⚠️ capturePicture 为同步 int 返回（0 成功/-1 失败，javap 5.28.4 确认），
     * 但部分设备文件异步写盘——返回值 0 后再以 300ms 步进轮询文件存在（最长 3s）。
     * SDK 无 MSG_PICTURE_SAVED 消息码，勿按 handler 回调等待。
     */
    class CaptureFrameSession(private val context: Context) {

        interface Listener {
            /** 抓帧成功（文件已落盘，可直接读取/上传；会话已回收） */
            fun onCaptured(file: File)
            fun onFailed(friendlyMessage: String)
        }
        ...
        private var released = false
        private var pollTimes = 0

        /** 播放成功 20s 未到则判定失败，避免会话悬挂 */
        private val timeoutRunnable = Runnable { fail("设备连接超时，抓拍失败，请重试") }

        /** 文件落盘轮询（capturePicture 返回 0 后仍可能异步写盘） */
        private val pollRunnable = object : Runnable {
            override fun run() {
                ...
                pollTimes++
                if (pollTimes >= PIC_POLL_MAX_TIMES) {
                    fail("抓拍失败，请重试")
                    return
                }
                mainHandler.postDelayed(this, PIC_POLL_INTERVAL_MS)
            }
        }
        ...
        private fun captureFrame() {
            ...
            try {
                ...
                picFile = file
                val ret = p.capturePicture(file.absolutePath)
                if (ret < 0) {
                    Log.e(TAG, "capturePicture 返回 $ret")
                    fail("抓拍失败，请重试")
                    return
                }
                // 返回值 0 仅表示命令接受，文件可能异步写盘：轮询确认
                pollTimes = 0
                mainHandler.postDelayed(pollRunnable, PIC_POLL_INTERVAL_MS)
            } catch (e: Exception) {
                Log.e(TAG, "抓帧异常", e)
                fail("抓拍失败，请重试")
            }
        ...
        /** 文件确认落盘 → 回收会话 → 回调成功 */
        private fun finishCaptured(file: File) {
            if (released) return
            released = true
            mainHandler.removeCallbacks(timeoutRunnable)
            ...
            player = null
            try {
                if (p != null) p.stopRealPlay()
            } catch (e: Exception) {
                Log.e(TAG, "停止抓帧会话异常", e)
            }
            try {
                if (p != null) EZOpenSDK.getInstance().releasePlayer(p)
            } catch (e: Exception) {
                Log.e(TAG, "释放抓帧播放器异常", e)
            }
        ...
        fun release() {
            ...
            try {
                ...
                Log.e(TAG, "回收抓帧会话异常", e)
            }
            try {
                if (p != null) EZOpenSDK.getInstance().releasePlayer(p)
            } catch (e: Exception) {
                Log.e(TAG, "回收抓帧播放器异常", e)
            }
            picFile?.delete()
            picFile = null
        }
        ...
        private companion object {
            const val PLAY_TIMEOUT_MS = 20_000L
            const val PIC_POLL_INTERVAL_MS = 300L
            const val PIC_POLL_MAX_TIMES = 10
        }
    }
